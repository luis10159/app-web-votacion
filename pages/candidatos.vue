...
let saveFacultad = async () => {
    ...
    console.log(candidatoName.value,indexPartido.value,indexFacultad.value ,"valore para enviar" )
    await ethereumService.addCandidate(candidatoName.value,indexPartido.value.idParty,indexFacultad.value.idFaculty,toast);
...
<template>
    ...
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" :value="candidatos" v-model:selection="selectedProducts" dataKey="idFaculty" 
                ...
                <Column field="idCandidate" header="Código" sortable style="min-width:12rem"></Column>
                <Column field="name" header="Nombre del Candidato" sortable style="min-width:16rem"></Column>
                <Column field="party.name" header="Nombre del partido" sortable style="min-width:16rem"></Column>
                <Column field="faculty.name" header="Nombre de la facultad" sortable style="min-width:16rem"></Column>
        ...
        <Dialog v-model:visible="productDialog" :style="{width: '450px'}" header="Nueva" :modal="true" class="p-fluid">
            ...
            <div class="field">
                <div >
                    ...
                    <InputText id="name" v-model.trim="candidatoName" required="true" autofocus :class="{'p-invalid': submitted && !candidatoName}" />
                    <small class="p-error mb-2" v-if="submitted && !candidatoName">Nombre es requerido</small>
                </div>
                <div class="mt-4">
                    <label for="name" >Ingrese Nueva Facultad</label>
                    <div class="card flex justify-content-center">
                        <Dropdown v-model="indexFacultad" :options="facultades" showClear optionLabel="name" placeholder="Seleccionar Facultad" class="w-full md:w-14rem" />
                    </div>
                ...
                <div class="mt-4">
                    ...
                    <small class="p-error" v-if="submitted && !indexPartido">Facultad es requerido</small>
                </div>
            </div>
